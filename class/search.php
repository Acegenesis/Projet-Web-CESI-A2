...
class search {
    ...
    function getperson($terme) {
        $stmt = $this->conn->prepare("SELECT name_promotion, name_campus, login, id_users FROM users JOIN promotion ON users.id_promotion = promotion.id_promotion JOIN campus ON promotion.id_campus = campus.id_campus JOIN address ON campus.id_address = address.id_address where login LIKE '%$terme%';");
        $stmt->execute();
        $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
        return $result;
    }
